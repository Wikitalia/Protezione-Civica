...
if (count($segnalazioni)) {
	...
	$xml_out.="<segnalazioni>";
		
	foreach ($segnalazioni as $segnalazione) {
	
		if ($segnalazione['id_tipo']) {
			$tipo_nome = $segnalazione['tipo_nome'];
		} elseif ($segnalazione['genere'] == 'buone-pratiche') {
		...
		$xml_out.="</longitudine>";
		$xml_out.="<descrizione>";
		$xml_out.=htmlspecialchars($segnalazione['messaggio'],ENT_QUOTES | ENT_XML1);
		$xml_out.="</descrizione>";
		$xml_out.="<via>";
		$xml_out.=htmlspecialchars($segnalazione['indirizzo'],ENT_QUOTES | ENT_XML1);
		$xml_out.="</via>";
		$xml_out.="<ncivico>";
		$xml_out.=htmlspecialchars($segnalazione['civico'],ENT_QUOTES | ENT_XML1);
		$xml_out.="</ncivico>";
		$xml_out.="<citta>";
		$xml_out.=htmlspecialchars($segnalazione['citta'],ENT_QUOTES | ENT_XML1);
		$xml_out.="</citta>";
		$xml_out.="<datasegnalazione>";
		$xml_out.=$segnalazione['data'];
		$xml_out.="</datasegnalazione>";
		$xml_out.="<urlfoto>";
		$xml_out.=htmlspecialchars($settings['sito']['url_ns'].$segnalazione['foto_base_url'].'300-0.jpg',ENT_QUOTES | ENT_XML1);
		$xml_out.="</urlfoto>";
		$xml_out.="<segnalatore>";
		$xml_out.=htmlspecialchars(trim($segnalazione['nome'].' '.$segnalazione['cognome']),ENT_QUOTES | ENT_XML1);
		$xml_out.="</segnalatore>";
		$xml_out.="<genere>";
		$xml_out.=$segnalazione['genere'];
		$xml_out.="</genere>";
		$xml_out.="<tipo_nome>";
		$xml_out.=$tipo_nome;
		$xml_out.="</tipo_nome>";
	...
	$xml_out.="<decorourbano><segnalazioni></segnalazioni></decorourbano>";
