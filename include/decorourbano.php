...
/**
 * Restituisce i tipi di segnalazioni ammessi dal sistema
 * 
 * Questa funzione restituisce i tipi di segnalazioni ammessi dal sistema
 * @return array lista degli enti che corrispondono all'id_comune inserito
 */
function tipi_get() {
	...
	foreach ($tipi as $k => $v) {
		...
		$tipi[$k]['tag'] = data_get('tab_tag', array('id_tipo' => $v['id_tipo']));
...
function segnalazioni_get($parametri) {
    ...
    $q_select = "SELECT s.id_segnalazione, s.id_tipo, s.lat, s.lng, s.id_comune, s.civico,
                 ...
                 u.id_ruolo, t.nome AS tipo_nome, tag.nome AS tag_nome, t.nome_url AS tipo_nome_url, 
    ...
    $q_from = " FROM tab_segnalazioni AS s
                ...
                LEFT JOIN tab_tipi AS t ON s.id_tipo = t.id_tipo
								LEFT JOIN tab_tag AS tag ON s.id_tag = tag.id_tag ";
...
function segnalazione_dettaglio_get($id) {
    ...
    $q = "SELECT s.*, u.id_fb, u.nome, u.cognome, u.mostra_cognome, u.id_ruolo, t.nome AS tipo_nome, tag.nome AS tag_nome, t.label AS tipo_label, e.id_ente, e.nome AS nome_ente,
    				cz.id_competenza as id_competenza, cz.nome as nome_competenza, cz.nome_url as nome_url_competenza, cz.stato as stato_competenza, cz.testo_mappa as testo_competenza, cz.testo_twitter as testo_twitter_competenza,
                (SELECT COUNT(*) 
...
function segnalazione_marker_url_get($segnalazione) {
	...
	if ($segnalazione['id_tipo'] == 0) {
		$marker = '/images/markerIcons/'.$segnalazione['genere'].'.png';
  } elseif ($segnalazione['id_competenza'] && $segnalazione['stato_competenza'] == 1) {
	  if ($segnalazione['stato'] >= 300) {
			$marker = '/images/markerIcons/'.$segnalazione['nome_url_competenza'].'.png';
	  ...
	  if ($segnalazione['stato'] >= 300) {
			$marker = '/images/markerIcons/'.$segnalazione['tipo_label'].'.png';
	  } else if ($segnalazione['stato'] >= 200) {
		  $marker = '/images/markerIcons/'.$segnalazione['tipo_label'].'.png';
	  } else {
		  $marker = '/images/markerIcons/'.$segnalazione['tipo_label'].'.png';
...
function segnalazione_fb_share($segnalazione) {
    ...
    $campi_per_fb = array(
        ...
        'description' => 'Le foto dei danni, le richieste e le offerte di aiuto, le strutture di emergenza e le buone pratiche di chi ha trovato un modo intelligente e generoso di affrontare il dopo terremoto: la mappa della protezione civica è tutto questo.',
