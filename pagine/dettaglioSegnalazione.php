...
if ($segnalazione) {
		...
		if ($segnalazione[0]['id_tipo']) {
			$tipo_nome = $segnalazione[0]['tipo_nome'];
		} elseif ($segnalazione[0]['genere'] == 'buone-pratiche') {
    ...
    if ($segnalazione[0]['stato'] >= $settings['segnalazioni']['in_attesa']) {
        ...
        $smarty->assign('pageTitle', $tipo_nome . ' a ' . $segnalazione[0]['citta'] . ' in ' . $segnalazione[0]['indirizzo']);
