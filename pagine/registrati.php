...
if (isset($_POST['form_registrazione'])) {
    ...
    $fields['nome'] = str_replace($settings['sito']['caratteri_non_ammessi'], '', $_POST['regNome']);
    $fields['cognome'] = str_replace($settings['sito']['caratteri_non_ammessi'], '', $_POST['regCognome']);
    ...
    if (!$errore_registrazione) {
        ...
        if ($id_utente) {
            ...
            $to = stripslashes(trim($fields['nome'] . ' ' . $fields['cognome'])) . ' <' . $fields['email'] . '>';
            $link = $settings['sito']['confermaRegistrazione'] . "?s=" . $email_code;

            $data['from'] = $from;
            $data['to'] = $to;
            $data['template'] = 'registrazioneConferma';
            $variabili['nome_utente'] = stripslashes(trim($fields['nome'] . ' ' . $fields['cognome']));
