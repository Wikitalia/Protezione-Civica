...
function aggiungi_segnalazione_lista(posizione, segnalazione) {
	...
	if (segnalazione.testo_twitter_competenza && segnalazione.testo_twitter_competenza != '')
		twit = '#protezionecivica : #'+segnalazione.tipo_nome+' #'+segnalazione.citta+' '+segnalazione.testo_twitter_competenza+' via @ProtezioneC';
	else
		twit = '#protezionecivica : #'+segnalazione.tipo_nome+' #'+segnalazione.citta+' via @ProtezioneC';
	...
	segnalazioneListaHTML='\
      ...
      <div class="auto right" style="width:120px"><fb:share-button href="'+segnalazione.url+'" type="button_count"></fb:like></div>\
			...
			<div class="segnDettagli">\
				<div class="segnDettagliLeft">\
					...
					if (segnalazione.genere != 'buone-pratiche')
						segnalazioneListaHTML += '<div class="fontS10 marginB5" style="float:left;width:180px;">{#categoria#}: '+segnalazione.tipo_nome+' - '+segnalazione.tag_nome+'</div>\
		...
		/*if (segnalazione.genere != 'buone-pratiche') {
			...
			segnalazioneListaHTML+='\
			<span id="nFollower_'+segnalazione.id_segnalazione+'">'+segnalazione.n_follower+'</span></div>';{*infoSegnalazione*}
		}*/
		segnalazioneListaHTML+='</div>\{*segnDettagli*}
		</div>\{*Senza nome 1*}
		...
		{/if}
			...
			if (segnalazione.foto == 1) {
				segnalazioneListaHTML+='<div class="segnImmagini" id="segnImmagini_'+segnalazione.id_segnalazione+'" onclick="showModal('+segnalazione.id_utente+', '+segnalazione.id_segnalazione+',\''+segnalazione.foto_base_url+'\');">\
					<div class="auto marginR10"><img src="{$settings.sito.url}'+segnalazione.foto_base_url+'585-440.jpg" /></div>\
				</div>\
		...
		if ('{$user.id_ruolo}' == '3')
			...
			<img src="{$settings.sito.url}images/commentFumettoComune.png" class="commentFumettino" />';
		else
			segnalazioneListaHTML+='<div class="commentoFumettoBox">\
			<img src="{$settings.sito.url}images/commentFumetto.png" class="commentFumettino" />';
		segnalazioneListaHTML+='\
...
function segnalazioni_nuove_get() {
	...
	if (!lock) {
		...
		$.ajax({
		{if $page == 'principale'}
			url: '{$settings.sito.url}ajax/segnalazioni_get.php?idu={$user.id_utente}&t_newer='+newest+'&c=1&w=1',
		{else if $page == 'vediProfilo'}
			url: '{$settings.sito.url}ajax/segnalazioni_get.php?idu={$user_profile.id_utente}&t_newer='+newest+'&c=1',
		{/if}
...
function segnalazioni_vecchie_get() {
	...
	if (!lock) {
		...
		$.ajax({
		{if $page == 'principale'}
			url: '{$settings.sito.url}ajax/segnalazioni_get.php?idu={$user.id_utente}&t_old='+oldest+'&l='+settings_limit_numero+'&c=1?w=1',
		{else if $page == 'vediProfilo'}
			url: '{$settings.sito.url}ajax/segnalazioni_get.php?idu={$user_profile.id_utente}&t_old='+oldest+'&l='+settings_limit_numero+'&c=1',
		{/if}
			dataType: "json",
			success: function(seg) {
...
function aggiungi_commento(ids,idu) {
	...
	if (commento != '')
		$.ajax({
			type: "POST",
		  url: '{$settings.sito.url}ajax/segnalazione_commento_add.php',
...
/*segnalazioneEdit(segnalazione.id_segnalazione,campo) {
	...
	$.ajax({
		type: "POST",
	  url: '{$settings.sito.url}ajax/segnalazione_edit.php',
	  data: 'ids='+ids,
	  success: function(data) {
			if (data == "1") {
...
function segnalazioneFollow(ids) {
	...
	$.ajax({
	  url: '{$settings.sito.url}ajax/segnalazione_follow.php?ids='+ids,
	  success: function(data) {
	  	if (data != "-1") {
...
function segnalazioneUnFollow(ids) {
	...
	$.ajax({
	  url: '{$settings.sito.url}ajax/segnalazione_unfollow.php?ids='+ids,
	  success: function(data) {
	  	if (data != "-1") {
	  		$('#nFollower_'+ids).html(data);
...
function commentoElimina(id) {
	...
	$.ajax({
	  url: '{$settings.sito.url}ajax/commento_elimina.php?id='+id,
	  success: function(data) {
			if (data == "1") {
			  $('#commento_'+id).remove();
			}
...
function commentoImproprio(idc,idu) {
	...
	$.ajax({
	  url: '{$settings.sito.url}ajax/commento_improprio.php?idc='+idc+'&idu='+idu,
	  success: function(data) {
			if (data == "1") {
			  alert('Grazie per il tuo contributo!');
			}
...
function showModal(idu, ids, foto_base_url) {
	...
	var image = $('<img />').attr('src', '{$settings.sito.url}'+foto_base_url+'0-0.jpg');
	image.load(function () {
  ...
  var image = new google.maps.MarkerImage('{$settings.sito.url}'+segnalazioni[ids].marker,
    new google.maps.Size(30, 48),
    new google.maps.Point(0,0),
    new google.maps.Point(15, 48));
