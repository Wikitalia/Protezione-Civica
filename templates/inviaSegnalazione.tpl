...
<div class="marginT10"><h3 class="fontS18 marginB5 fBrown">{#titolo#}</h3>
		{#inviaIntro#} 
	</div>
<form id="formInviaSegnalazione" onsubmit="return ajaxSubmit();">
	...
	<div id="invSegnCategoria" class="testoFumetto">
		...
		<input type="radio" id="radio{$tipo.id_tipo}" name="tipo" class="" value="{$tipo.id_tipo}" onclick="categoria_set({$tipo.id_tipo});" />
		<label for="radio{$tipo.id_tipo}" aria-pressed="false" class="categoriaButt" role="button" aria-disabled="false">
			<img src="{$settings.sito.url}images/markerIcons/{$tipo.label}.png" alt="" title="{$tipo.nome}" /><br />
			<span class="fontS9">{$tipo.nome}</span>
		</label>
		{/foreach}
	</div>
	...
	<div id="invSegnSottoCategoria" class="testoFumetto marginB20"> 
		...
		{foreach from=$tipi item=tipo}
		<select name="id_tag" id="select_tag_{$tipo.id_tipo}" style="display:none;">
			{foreach from=$tipo['tag'] item=tag}
			<option value="{$tag.id_tag}">{$tag.nome}</option>
			{/foreach}
		</select>
		{/foreach}
	</div>
	...
	<div id="invSegnForm" class="skinnedForm">
		<h3 class="pageTitle marginB5"><span class="fontS20">3.</span> {#descProblema1#} <span id="controlloDescrizione"></span></h3> 
		<div>
			<div>
				<b>{#descrizione#} *</b><br />
		...
		<div class="marginT50 marginB20"><input type="submit" value="{#inviaButt#}" /></div>
	</div>
	
	<div id="invSegnMappa">
		<h3 class="pageTitle marginB5"><span class="fontS20">4.</span> {#affinaPos#} <span id="controlloMappa"></span></h3>
  ...
  $.ajax({
    ...
    success: function(result) {
    	if (result && result.status == 'ok') {
	    	...
	    	$('#riepilogoFoto').attr('src','/resize.php?w=450&f={$settings.sito.url}images/segnalazioni/{$user.id_utente}/'+result+'/1.jpeg');						    	
